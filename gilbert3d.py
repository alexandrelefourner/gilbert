...
Adapted by Alexandre A. LE FOURNER to get a numpy array with Python 3
"""
import numpy as np
...
def gilbert3d(x, y, z,
    ...
    if h == 1 and d == 1:
        for i in range(0, w):
            mlist.append((int(x), int(y), int(z)))
    ...
    if (2*w > 3*h) and (2*w > 3*d):
            mlist = mlist + gilbert3d(x, y, z,
                ax2, ay2, az2,
                bx, by, bz,
                cx, cy, cz)

            mlist = mlist + gilbert3d(x+ax2, y+ay2, z+az2,
    ...
    # do not split in d
            ...
            mlist = mlist + gilbert3d(x+bx2, y+by2, z+bz2,
                ax, ay, az,
                bx-bx2, by-by2, bz-bz2,
                cx, cy, cz)

            mlist = mlist + gilbert3d(x+(ax-dax)+(bx2-dbx),
                y+(ay-day)+(by2-dby),
                z+(az-daz)+(bz2-dbz),
                -bx2, -by2, -bz2,
                cx, cy, cz,
    ...
    # do not split in h
            ...
            mlist = mlist + gilbert3d(x+cx2, y+cy2, z+cz2,
                ax, ay, az,
                bx, by, bz,
                cx-cx2, cy-cy2, cz-cz2)

            mlist = mlist + gilbert3d(x+(ax-dax)+(cx2-dcx),
                y+(ay-day)+(cy2-dcy),
                z+(az-daz)+(cz2-dcz),
                -cx2, -cy2, -cz2,
                -(ax-ax2), -(ay-ay2), -(az-az2),
    ...
    # regular case, split in all w/h/d
            ...
            mlist = mlist + gilbert3d(x+(bx2-dbx)+(cx-dcx),
                y+(by2-dby)+(cy-dcy),
                z+(bz2-dbz)+(cz-dcz),
                ax, ay, az,
                -bx2, -by2, -bz2,
            ...
            mlist = mlist + gilbert3d(x+(ax-dax)+bx2+(cx-dcx),
                y+(ay-day)+by2+(cy-dcy),
                z+(az-daz)+bz2+(cz-dcz),
                -cx, -cy, -cz,
                -(ax-ax2), -(ay-ay2), -(az-az2),
